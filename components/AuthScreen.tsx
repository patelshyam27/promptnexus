...
import { Zap, ArrowRight, UserPlus, LogIn, Loader2, User as UserIcon, Eye, EyeOff, X } from 'lucide-react';
...
const AuthScreen: React.FC<AuthScreenProps> = ({ onAuthSuccess, isModal = false, onClose }) => {
  ...
  const containerClasses = isModal
    ? "bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl p-6 relative max-h-[90vh] overflow-y-auto"
    : "min-h-screen bg-black flex flex-col items-center justify-center p-4 relative overflow-hidden";

  const formWrapperClasses = isModal
    ? "w-full"
    : "w-full max-w-md bg-slate-900/50 border border-slate-800 backdrop-blur-xl rounded-2xl p-8 shadow-2xl z-10 transition-all duration-300";

  return (
    <div className={isModal ? "fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" : containerClasses}>
      ...
      <div className={isModal ? containerClasses : "w-full flex flex-col items-center"}>

        {isModal && onClose && (
          <button onClick={onClose} className="absolute right-4 top-4 text-slate-400 hover:text-white z-20">
            <X size={24} />
          </button>
        )}
        ...
        <div className="flex flex-col items-center mb-8 z-10">
          <div className="bg-gradient-to-br from-primary-500 to-primary-700 p-3 rounded-xl shadow-2xl shadow-primary-500/30 mb-4">
            <Zap size={isModal ? 32 : 40} className="text-white" fill="white" />
          </div>
          <h1 className={`${isModal ? "text-2xl" : "text-4xl"} font-bold text-white tracking-tight`}>PromptNexus</h1>
